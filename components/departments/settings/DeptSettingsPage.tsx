...
import { User, Palette, BellRing, ShieldCheck, Settings2 } from 'lucide-react';
import { useDepartmentProfile } from '@/components/departments/contexts/DepartmentProfileContext';

import DeptMyProfileTab from '@/components/departments/settings/DeptMyProfileTab';
import DeptColorThemeTab from '@/components/departments/settings/DeptColorThemeTab';
import DeptNotificationTab from '@/components/departments/settings/DeptNotificationTab';
import DeptSecurityTab from '@/components/departments/settings/DeptSecurityTab';
import DeptSystemPreferencesTab from '@/components/departments/settings/DeptSystemPreferencesTab';
...
const TAB_MAP: Record<string, React.ReactNode> = {
  profile: <DeptMyProfileTab />,
  theme: <DeptColorThemeTab />,
  notifications: <DeptNotificationTab />,
  security: <DeptSecurityTab />,
  system: <DeptSystemPreferencesTab />,
};
...
export default function DeptSettingsPage() {
  ...
  const go = (value: string) =>
    router.push(`/departments/settings?tab=${value}`, { scroll: false });
    ...
    <div className="max-w-[1400px] mx-auto space-y-4">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Manage your profile, appearance, and department preferences.
        </p>
      </div>
      ...
      <div className="md:hidden w-full">
        ...
        <div className="mt-4 w-full">{TAB_MAP[activeTab] ?? <DeptMyProfileTab />}</div>
      </div>
      ...
      <div className="hidden md:flex gap-6 items-start">
        ...
        <div className="flex-1 min-w-0 w-full">{TAB_MAP[activeTab] ?? <DeptMyProfileTab />}</div>
      </div>
    </div>
  );
}
