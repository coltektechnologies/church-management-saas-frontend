...
/**
 * After JWT auth: only secretary roles (or platform admin) may use `/secretary`.
 * Blocks manual URL entry the same way as {@link RequireTreasuryPortal}.
 */
export function RequireSecretaryPortal({ children }: { children: React.ReactNode }) {
  const router = useRouter();
  const [allowed, setAllowed] = useState(false);

  useEffect(() => {
    const user = storedUserToPostLogin(getStoredUser());
    if (!userHasSecretaryPortalAccess(user)) {
