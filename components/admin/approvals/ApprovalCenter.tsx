...
export default function ApprovalCenter({ variant = 'admin' }: ApprovalCenterProps) {
  ...
  const loadAll = useCallback(async () => {
    ...
    let programRows: ProgramListItem[] = [];
    if (isSecretary) {
      programRows = await fetchProgramsByStatus(PROGRAM_SECRETARIAT_QUEUE_STATUS);
    } else {
      programRows = await Promise.all(
        PROGRAM_PENDING_STATUSES_ADMIN.map((s) => fetchProgramsByStatus(s))
      ).then((groups) => groups.flat());
    }

    let expenseRows: ExpenseRequestItem[] = [];
    if (!isSecretary) {
