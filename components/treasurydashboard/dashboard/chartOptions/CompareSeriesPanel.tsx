    ...
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10, gap: 8, flexWrap: 'wrap' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <span style={{ fontSize: 9, color: `${textColor}40`, fontFamily: "'OV Soge', sans-serif" }}>
            {selectedTypes.length} of {allTypes.filter((t) => !t.isHidden).length} active selected
          </span>
        </div>
        {selectedTypes.length > 0 && (
          <span style={{ fontSize: 9, color: accentColor, fontFamily: "'OV Soge', sans-serif", fontWeight: 700 }}>
      ...
      <div style={{ position: 'relative', marginBottom: 10 }}>
        <Search size={12} style={{ position: 'absolute', left: 9, top: '50%', transform: 'translateY(-50%)', color: `${textColor}45`, pointerEvents: 'none' }} />
        <input
          type="text"
          value={query}
        ...
        {query && (
          <button
            type="button"
            onClick={() => setQuery('')}
            style={{ position: 'absolute', right: 8, top: '50%', transform: 'translateY(-50%)', backgroundColor: 'transparent', border: 'none', cursor: 'pointer', color: `${textColor}50`, display: 'flex', padding: 2 }}
          >
            <X size={11} />
          </button>
        )}
      ...
      {/* Type pills */}
      {displayed.length === 0 ? (
        <span style={{ fontSize: 11, color: `${textColor}40`, fontFamily: "'OV Soge', sans-serif" }}>
          ...
          {displayed.map((t) => {
            ...
            return (
              <button
                ...
                title={isHidden ? `"${t.label}" is hidden from the form but can still be compared` : undefined}
                style={{
                  ...
                  backgroundColor: active ? `${col}1A` : isHidden ? `${textColor}05` : 'transparent',
      ...
      {allTypes.some((t) => t.isHidden) && (
        <p style={{ fontSize: 9, color: `${textColor}35`, fontFamily: "'OV Soge', sans-serif", marginTop: 8, lineHeight: 1.4 }}>
