...
export function staffDisplayName(
  m: Pick<StaffMember, 'first_name' | 'last_name' | 'email'>
): string {
...
export function groupLabels(ids: string[], catalog?: ChurchGroupOption[]): string[] {
  const src: ChurchGroupOption[] = catalog && catalog.length > 0 ? catalog : MOCK_CHURCH_GROUPS;
  return ids.map((id) => src.find((g) => g.id === id)?.name).filter((x): x is string => Boolean(x));
}
