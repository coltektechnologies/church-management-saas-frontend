...
export default function AppearancePanel({
  showGrid, gridStyle, showLabels, showBorder, legendPos,
  onShowGrid, onGridStyle, onShowLabels, onShowBorder, onLegendPos,
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
        ...
        <button type="button" onClick={() => onShowGrid(!showGrid)}
        ...
        <button type="button" onClick={() => onShowLabels(!showLabels)}
          style={pill(showLabels, accentColor, borderColor, textColor)}>
          Data Labels {showLabels ? 'On' : 'Off'}
        </button>

        <button type="button" onClick={() => onShowBorder(!showBorder)}
        ...
        <div style={{ display: 'flex', gap: 4, alignItems: 'center', flexWrap: 'wrap' }}>
          <span style={{
            fontSize: 9, color: `${textColor}50`, fontWeight: 700,
          ...
          {(['bottom', 'top', 'none'] as const).map(pos => (
            <button
              ...
              style={{
                ...
                textTransform: 'capitalize', padding: '4px 8px',
              }}
            >
              {pos}
            </button>
