  ...
  return (
    <div
      style={{
        ...
        border: `1px solid ${editing ? '#F59E0B' : borderColor}`,
        borderRadius: '10px',
        boxShadow: editing
          ? '0 0 0 2px rgba(245,158,11,0.22), 0 2px 10px rgba(0,0,0,0.08)'
      ...
      {editing && (
        <div
          style={{
            ...
            gap: '8px',
            padding: '8px 14px',
            backgroundColor: justEntered ? '#FEF08A' : '#FEF3C7',
        ...
        >
          ...
          >
            Editing this record — change the highlighted fields below, then click{' '}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', minWidth: 0, flex: 1 }}>
          <div
            style={{
              ...
              flexShrink: 0,
              backgroundColor: editing ? '#FEF3C7' : `${accentColor}20`,
            }}
          >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '6px', flexShrink: 0 }}>
          ...
          <button
            ...
            style={{
              ...
              alignItems: 'center',
              justifyContent: 'center',
              backgroundColor: editing ? '#F59E0B' : `${tc}12`,
              color: editing ? '#fff' : tc,
              border: 'none',
              cursor: 'pointer',
              boxShadow: editing ? '0 0 0 3px rgba(245,158,11,0.28)' : 'none',
              transition: 'all 0.2s',
            }}
          >
            <Edit2 size={13} />
      ...
      {expanded && (
        <div
          style={{
            ...
            borderTop: `1px solid ${editing ? '#FDE68A' : borderColor}`,
          }}
        >
          {editing && (
            <div
          ...
          >
            <EditableField
              label="Date"
              value={editing ? draft.date : record.date}
              editing={editing}
            ...
            <EditableField
              label="Amount"
              value={
                editing ? String(draft.amount) : formatCurrency(record.amount, record.currency)
              }
              editing={editing}
              onChange={(v) =>
                setDraft((p) => ({ ...p, amount: Number(v.replace(/[^0-9.]/g, '')) || 0 }))
              }
              textColor={tc}
              accentColor={accentColor}
            />
            <EditableField
              ...
              value={`${record.paymentMethod}${record.paymentDetail ? ` (${record.paymentDetail})` : ''}`}
              editing={false}
              onChange={() => {}}
              textColor={tc}
              accentColor={accentColor}
            ...
            <EditableField
              label="Notification"
              value={record.notification.join(', ') || '—'}
              editing={false}
              onChange={() => {}}
              textColor={tc}
              accentColor={accentColor}
...
function doExport(
  ...
  if (format === 'json') {
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = 'income_records.json';
    a.click();
    URL.revokeObjectURL(url);
    return;
  }
      ...
      >
        <div style={{ position: 'relative', flex: 1, minWidth: '180px' }}>
          ...
          <input
            ...
            placeholder="Search name, type, description, receipt…"
        ...
        <button
          ...
          style={{
            ...
            backgroundColor:
              showFilter || filterFrom || filterTo ? `${accentColor}15` : 'transparent',
            color: showFilter || filterFrom || filterTo ? accentColor : `${textColor}70`,
          }}
        >
        ...
        <div ref={exportRef} style={{ position: 'relative' }}>
          ...
          >
            ...
            <ChevronRight
              size={11}
              style={{
                transform: showExport ? 'rotate(90deg)' : 'rotate(0)',
      ...
      {typesInRecords.length > 0 && (
        <div style={{ marginBottom: '12px' }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
            <button onClick={() => setFilterType('')} style={pillStyle(!filterType)}>
              All
            </button>
              ...
              <>
                ...
                {otherDetails.map((detail) => {
                  ...
                  return (
                    <button
                      key={detail}
                      onClick={() => setFilterType(filterType === detailKey ? 'other' : detailKey)}
                      style={pillStyle(filterType === detailKey, true)}
                      title={`Show only: Other — ${detail}`}
                    >
                      {detail}
                    </button>
                  );
