...
export const CHART_FAMILIES: Family[] = [
  ...
  },
  {
    ...
    subs: [
      ...
      { id: 'line-stepped', label: 'Step Line',      icon: '⌐',  desc: 'Staircase / stepped transitions'  },
      { id: 'line-dots',    label: 'Line + Markers', icon: '•╱', desc: 'Dots plotted at every data point' },
    ],
  },
  {
    ...
    subs: [
      ...
      { id: 'area-stacked', label: 'Stacked Area',  icon: '≡',  desc: 'Multiple series stacked on top'     },
  ...
  {
    id: 'radialbar', label: 'Radial', icon: '◎',
    subs: [
...
function pill(active: boolean, accent: string, border: string, text: string): React.CSSProperties {
  return {
    ...
    fontSize: 10, fontFamily: "'OV Soge',sans-serif", fontWeight: active ? 700 : 500,
...
export default function ChartTypePanel({
  chartType, activeFamily, onChartType, onFamily,
      ...
      <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap', marginBottom: 10 }}>
        {CHART_FAMILIES.map(f => (
          <button
            key={f.id}
            type="button"
            onClick={() => { onFamily(f.id); onChartType(f.subs[0].id); }}
            style={{
      ...
      <div style={{
        ...
        {currentFamily.subs.map(s => (
          <button
            ...
            style={{
              padding: '8px 11px', borderRadius: 8, textAlign: 'left', cursor: 'pointer',
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 2 }}>
              ...
              <span style={{ fontSize: 11, fontWeight: 700, color: chartType === s.id ? accentColor : textColor }}>
                {s.label}
              </span>
              {chartType === s.id && <Check size={9} style={{ color: accentColor, marginLeft: 'auto' }} />}
            </div>
