...
const MENU_ITEMS: MenuItem[] = [
  { icon: User, label: 'Your profile', href: '/secretary/settings?tab=profile' },
  { icon: Settings, label: 'Account settings', href: '/secretary/settings?tab=system' },
  { icon: Languages, label: 'Language', href: '/secretary/settings?tab=system' },
  { icon: Shield, label: 'Security', href: '/secretary/settings?tab=security' },
...
export default function UserDropdown({
  triggerName = 'User',
  initials = 'U',
  // Props received from parent but resolved internally from context — kept for
  // API compatibility; prefixed with _ to satisfy the no-unused-vars rule.
  ...
  const churchName = churchReady
    ...
    : 'SDA Church - Adenta';
  const dark = churchReady ? church.theme === 'dark' || church.darkMode : false;
  const displayName = userReady ? user.adminName || 'User' : 'User';
  const displayRole = userReady ? user.adminRole || 'Secretary' : 'Secretary';
