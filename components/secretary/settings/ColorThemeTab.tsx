...
// Default palette — white sidebar + white topbar
...
function DashboardPreview({
  ...
  const NAV = ['Dashboard', 'Members', 'Treasury', 'Reports', 'Settings'];
...
export default function ColorThemeTab() {
  const { profile, updateProfile, toggleDarkMode, isReady } = useSecretaryProfile();
  const isDark = isReady ? profile.darkMode : false;

  // Initialised ref — ensures we only sync from profile once (on first ready tick).
  // This avoids calling setState synchronously inside an effect body, which
  // ESLint flags as react-hooks/set-state-in-effect.
  ...
  useEffect(() => {
    ...
    // Schedule state update — not synchronous within the effect body
    setLight({
  ...
  }, [isReady]); // single initialisation when profile becomes ready

  const activeColors = isDark ? dark : light;

  // Palette selection — sets ALL light colours at once
  ...
  // Custom colour change — sidebar mirrors to topbar when they are in sync
  ...
  const handleSave = useCallback(() => {
    ...
    setSaved(true);
    toast.success('Theme saved!', { description: 'Sidebar, topbar, and all colours updated.' });
    setTimeout(() => setSaved(false), 2000);
  }, [light, dark, updateProfile]);

  // Reset restores the Default palette (white sidebar + white topbar)
  ...
  // Current colours exposed to PaletteGrid's Custom tab
