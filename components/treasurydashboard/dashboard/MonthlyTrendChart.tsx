...
import { getOptions, type DropdownOption } from '@/components/treasurydashboard/recordIncome/dropdownOptions';
...
const TYPE_BASE: Record<string, number> = {
  tithe: 14_500, offering: 9_800, thanksgiving: 6_200,
  harvest: 4_100, welfare: 2_800, other: 3_500,
};
const TYPE_SEED: Record<string, number> = {
  tithe: 7341, offering: 2891, thanksgiving: 5612,
  harvest: 3309, welfare: 1247, other: 8823,
};
...
function buildMonthlyData(typeValues: string[]): Record<string, number | string>[] {
  return MONTHS.map((month, mi) => {
    ...
    typeValues.forEach(tv => {
      ...
      row[tv] = Math.max(800, Math.round(base + seasonal + (rng() - 0.48) * variance + decBoost + aprBoost));
    });
    return row;
  });
}
...
function filterByIsoRange(
  ...
  if (!range.from && !range.to) {return allData;} // "all"
...
function ChartTooltip({
  active, payload, label, cardBg, textColor, liveTypes,
}: {
  active?:   boolean;
  ...
  return (
    <div style={{
      background: cardBg, border: '1px solid rgba(0,0,0,0.1)',
      ...
      {payload.map(item => (
        <div key={item.name} style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 3 }}>
          <span style={{ width: 8, height: 8, borderRadius: '50%', backgroundColor: item.color, flexShrink: 0, display: 'inline-block' }} />
...
// Active panel type
// ─────────────────────────────────────────────────────────────────────────────
type ActivePanel = 'chartType' | 'compareSeries' | 'dateRange' | 'appearance' | 'colorSeries' | null;
...
export default function MonthlyTrendChart() {
  ...
  const {
    chartType, activeFamily, selectedTypes, rangeMode, preset,
    customFrom, customTo, showGrid, gridStyle, showLabels,
    legendPos, showBorder, seriesColors,
  } = prefs;
  ...
  useEffect(() => {
    if (!liveTypes.length || !prefsLoaded) {return;}
    ...
    setPrefs(prev => {
      ...
      if (JSON.stringify(next) === JSON.stringify(prev.selectedTypes)) {return prev;}
      return { ...prev, selectedTypes: next };
    });
    setPrefs(prev => {
      const nextColors = { ...prev.seriesColors };
      liveTypes.forEach((t, i) => {
        if (!nextColors[t.value]) {nextColors[t.value] = PALETTE[i % PALETTE.length];}
      });
  ...
  const heading = useMemo(() => {
    ...
    if (!labels.length)      {return 'Income Trend';}
    if (labels.length === 1) {return `${labels[0]} — Monthly Trend`;}
  ...
  const subheading = useMemo(() => {
    ...
    const typeLabel = CHART_FAMILIES.flatMap(f => f.subs).find(s => s.id === chartType)?.label ?? chartType;
  ...
  // ── Legend ────────────────────────────────────────────────────────────────
  const commonLegend = legendPos !== 'none' ? (
    <Legend
      ...
      formatter={v => <span style={{ color: textColor, fontSize: 10 }}>{getTypeLabel(String(v))}</span>}
    />
  ) : undefined;
  ...
  const renderChart = () => {
          ...
          {isH ? (
            <>
              <XAxis
                ...
                width={44}
              />
              <YAxis type="category" dataKey="month" tick={axisStyle} axisLine={false} tickLine={false} width={36} />
            </>
          ) : (
            <>
    ...
    // ── LINE ─────────────────────────────────────────────────────────────────
    if (family === 'line') {
      const curve   = chartType === 'line-smooth' ? 'monotone' : chartType === 'line-stepped' ? 'stepAfter' : 'linear';
      const showDot = chartType === 'line-dots';
      return (
        <LineChart data={filteredData} margin={{ top: showLabels ? 18 : 4, right: 8, left: 0, bottom: 0 }}>
          ...
          {selectedTypes.map((tv, i) => (
            <Line
              key={tv} type={curve as 'linear'} dataKey={tv} name={tv}
              stroke={getColor(tv, i)} strokeWidth={2.5}
    ...
    if (family === 'area') {
      ...
      return (
        <AreaChart data={filteredData} margin={{ top: showLabels ? 18 : 4, right: 8, left: 0, bottom: 0 }}>
          <defs>
            {selectedTypes.map((tv, i) => {
          ...
          {selectedTypes.map((tv, i) => (
            <Area
              key={tv} type={curve as 'monotone'} dataKey={tv} name={tv}
              stroke={getColor(tv, i)} strokeWidth={2}
              fill={`url(#grad-${tv})`} fillOpacity={1}
    ...
    if (family === 'pie') {
      ...
      return (
        <PieChart margin={{ top: 4, right: 8, bottom: 4, left: 8 }}>
          ...
          <Pie
            data={aggregated}
            cx="50%" cy={isSemi ? '72%' : '50%'}
            innerRadius={innerR} outerRadius="75%"
    ...
    if (family === 'radar') {
      ...
      return (
        <RadarChart data={filteredData} margin={{ top: 8, right: 20, bottom: 8, left: 20 }}>
          ...
          {selectedTypes.map((tv, i) => (
            <Radar
              key={tv} name={tv} dataKey={tv}
              stroke={getColor(tv, i)} strokeWidth={2}
              fill={getColor(tv, i)} fillOpacity={isFilled ? 0.22 : 0}
            />
          ))}
        </RadarChart>
      );
    ...
    if (family === 'radialbar') {
      ...
      return (
        <RadialBarChart
          ...
          data={rData} startAngle={180} endAngle={0}
          margin={{ top: 0, right: 40, bottom: 0, left: 40 }}
        >
          <RadialBar
            dataKey="pct" cornerRadius={4}
            label={showLabels
              ? { position: 'insideStart', fill: '#fff', fontSize: 10, fontFamily: "'OV Soge',sans-serif" }
              : false}
          />
          <Tooltip content={tooltipContent} />
          {commonLegend}
  ...
  const rangeSummaryLabel = rangeMode === 'preset'
    ? (TREASURY_PRESETS.find(p => p.value === preset)?.label ?? 'All Time')
    : (customFrom && customTo ? `${customFrom} → ${customTo}` : 'Custom Range');
      ...
      <div style={{
        ...
        <div style={{ minWidth: 0, flex: 1 }}>
          <h3 style={{
            ...
            fontSize: 'clamp(12px, 1.8vw, 14px)', color: textColor, margin: 0,
            overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap',
          }}>
            {heading}
          </h3>
          <p style={{ fontSize: 10, color: `${textColor}55`, margin: '2px 0 0' }}>
            {subheading}
          </p>
        </div>
      ...
      <div style={{ padding: '14px 18px 6px' }}>
        {selectedTypes.length === 0 ? (
          <div style={{
            ...
            <p style={{ fontSize: 12, color: `${textColor}50`, fontFamily: "'OV Soge', sans-serif" }}>
      ...
      {summaryTotals.length > 0 && (
        <div style={{ padding: '10px 18px 14px', display: 'flex', flexWrap: 'wrap', gap: 10 }}>
          {summaryTotals.map(s => (
            <div key={s.label} style={{
              ...
              <span style={{
                ...
                backgroundColor: s.color, flexShrink: 0, display: 'inline-block',
              }} />
              <div>
                <div style={{
                  fontSize: 9, color: `${textColor}60`, fontWeight: 700,
                  textTransform: 'uppercase', letterSpacing: '0.05em',
                }}>
                  {s.label}
                </div>
                <div style={{ fontSize: 13, fontWeight: 700, color: textColor, fontFamily: "'Poppins',sans-serif" }}>
          ...
          <div style={{ marginLeft: 'auto', alignSelf: 'center' }}>
            <span style={{
              fontSize: 9, color: `${textColor}40`, fontWeight: 600,
